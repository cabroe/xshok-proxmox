...
echo "wget https://raw.githubusercontent.com/cabroe/xshok-proxmox/master/install-post.sh -c -O install-post.sh && bash install-post.sh && rm install-post.sh" >> /post-install
